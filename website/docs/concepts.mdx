...
```csharp
/// <summary>The naive shape: the effect runs above the pause, so it re-runs on resume.</summary>
...
```csharp
/// <summary>The fix: each effect is a journaled step, so replay never re-runs it.</summary>
.Node("checkout", async (state, ctx) =>
{
    // Called once, ever. On the resume pass this returns the journaled order.
...
```csharp
.Node("charge_then_call", fn, retry: new RetryPolicy { MaxAttempts = 3, Backoff = TimeSpan.FromMilliseconds(200), Factor = 2 })
```
